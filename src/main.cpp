...
void compute_fisher_encoding_and_save(string filename, std::vector<float> &vec_enc, VlGMM* &gmm, DESC_TYPE desc_type, vl_size numClusters)
{
    cout << "\nFV encoding__________________________________________________" << endl;

    // Load image descriptor
    vector<KeyPoint> l_kpts; Mat l_desc;    
...
vl_size load_all_descriptors(vector<cv::String> &images_list, float* &data, int gmm_words, int count, DESC_TYPE desc_type, vl_size dimension)
{
    ...
    {
        // Load descriptors
        Mat l_desc; vector<KeyPoint> l_kpts; 
...
void example_get_indexing(string in_folder, int gmm_words, DESC_TYPE desc_type, vl_size dimension, vl_size numClusters)
{
...
void get_knn(string img_query, string in_folder, int k, VlGMM* &gmm, DESC_TYPE desc_type, vl_size numClusters, vl_size dimension, bool load)
{
...
void encoding_similarity_test(string img1, string img2, vector<pair<double, int> > &l2, vector<pair<double, int> > &euclidean, int index)
{
    // Load FV
...
void get_k_brute_force(string img_query, string in_folder, int k, VlGMM* &gmm, DESC_TYPE desc_type, vl_size numClusters, vl_size dimension)
{
    // Compute FV for query image 
    //load_fisher_encoding(enc_query, img_query);
    vector<float> enc_query;
    ...
    cout << "\nBrute force:" << endl;
    for (size_t i=0; i<count; i++)
        encoding_similarity_test(img_query, images_list[i], l2, euc, i);
...
void example_query(string query_img, string in_folder, DESC_TYPE desc_type, vl_size numClusters, vl_size dimension, bool load_index)
{
    bool success = false;
    if( desc_type == SIFT_DESC ){
...
void help(){
    cout << "Modes: descriptors | indexing | knn.\nIf 'descriptors': \nYou must provide {'kaze' | 'sift'} <dataset_folder> as argument. \nIf 'indexing': \nYou must provide {'kaze' | 'sift'} <dataset_folder> as argument. \nIf 'knn': \nYou must provide {'kaze' | 'sift'} <dataset_folder> <query_image> {'save' | 'load'} as arguments where 'save' means store kdtree index (to be used first time), and 'load' will read it from memory. \n" << endl;
}
